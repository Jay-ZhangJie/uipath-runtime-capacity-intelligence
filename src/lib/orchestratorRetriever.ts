...
type LivePagedRawProbeResult = LiveRawProbeResult & { attempts: string[]; label: string; pageResults: LiveRawProbeResult[] };
...
const tenantFoldersLabel = 'odata/Folders tenant-wide';

const currentUserRoleCandidates: EndpointCandidate[] = [
  {
    path: 'odata/Users/UiPath.Server.Configuration.OData.GetCurrentUser',
    query: '$expand=Roles',
    label: 'current user roles with Roles expand',
  },
  {
    path: 'odata/Users/UiPath.Server.Configuration.OData.GetCurrentUser()',
    query: '$expand=Roles',
    label: 'current user roles with method parentheses',
  },
  {
    path: 'odata/Users/UiPath.Server.Configuration.OData.GetCurrentUser',
    query: '',
  ...
  },
  {
    path: 'odata/Users/UiPath.Server.Configuration.OData.GetCurrentUser()',
    query: '',
    label: 'current user profile with method parentheses',
  },
];
...
const tenantFolderQuery = [
  '$select=Id,Key,DisplayName,FullyQualifiedName,Name,ParentId,ParentKey',
  `$top=${odataPageSize}`,
  '$orderby=FullyQualifiedName asc',
].join('&');
const tenantFolderFallbackQuery = `$top=${odataPageSize}&$orderby=FullyQualifiedName asc`;
...
function collectRoleRecords(value: unknown, roleRecords: RawRecord[] = []): RawRecord[] {
  if (!value || typeof value !== 'object') return roleRecords;
  if (Array.isArray(value)) {
    value.forEach((item) => collectRoleRecords(item, roleRecords));
    return roleRecords;
  }
  ...
  if (hasRoleShape) roleRecords.push(record);

  Object.entries(record).forEach(([key, nested]) => {
    if (/role/i.test(key) || key === 'value' || key === 'Value' || key === 'data' || key === 'Data') {
      collectRoleRecords(nested, roleRecords);
    }
  });
...
function roleNameFromRecord(record: RawRecord) {
  return textValue(firstValue(record, ['Name', 'name', 'DisplayName', 'displayName', 'RoleName', 'roleName', 'Role', 'role']));
}

function roleScopeFromRecord(record: RawRecord) {
...
function roleIsTenantAdmin(record: RawRecord) {
  const normalizedName = roleNameFromRecord(record).toLowerCase().replace(/[_-]+/g, ' ').replace(/\s+/g, ' ').trim();
  return normalizedName === 'orchestrator administrator';
}

function tenantAdminInfoFromPayload(payload: RawRecord) {
  const roleRecords = collectRoleRecords(payload);
  const roleNames = uniqueSorted([
    ...roleRecords.map(roleNameFromRecord),
    ...stringListValue(firstValue(payload, ['RoleNames', 'roleNames', 'RolesList', 'rolesList'])),
  ]);
  const tenantAdmin = roleRecords.some(roleIsTenantAdmin) || roleNames.some((name) =>
    name.toLowerCase().replace(/[_-]+/g, ' ').replace(/\s+/g, ' ').trim() === 'orchestrator administrator'
  );

  return { tenantAdmin, roleNames };
}
...
async function runFolderedPagedODataProbe(
  config: ProbeConnectionConfig,
  token: string | undefined,
  folders: LiveFolderSummary[],
  entitySet: string,
  query: string,
  fallbackQuery: string,
): Promise<Array<{ folder: LiveFolderSummary; result: LivePagedRawProbeResult }>> {
  const settled = await settledBatch(
    folders,
    10,
    async (folder): Promise<{ folder: LiveFolderSummary; result: LivePagedRawProbeResult }> => ({
      folder,
      result: await runPagedODataProbeWithFallback(
        config,
        token,
        entitySet,
        query,
  ...
  return settled.map((result, index) => (
    result.status === 'fulfilled'
      ? {
        ...
        result: {
          ...result.value.result,
          pageResults: result.value.result.pageResults.map((pageResult) => withFolderDiagnostic({
            folder: result.value.folder,
            result: pageResult,
          }).result),
        },
      ...
      : {
        ...
        result: {
          ...
          attempts: [errorMessage(result.reason)],
          label: entitySet,
          pageResults: [],
        },
      }
...
async function runPagedODataProbeWithFallback(
  config: ProbeConnectionConfig,
  token: string | undefined,
  entitySet: string,
  selectedFieldQuery: string,
  fallbackQuery: string,
  folderId?: number,
  label = `odata/${entitySet}`,
): Promise<LivePagedRawProbeResult> {
  ...
  for (let pageIndex = 0; pageIndex < maxCurrentUserFolderPages; pageIndex += 1) {
    const skip = pageIndex * odataPageSize;
    const result = await runODataProbeWithFallback(
      config,
      token,
      entitySet,
      `${selectedFieldQuery}&$skip=${skip}`,
      `${fallbackQuery}&$skip=${skip}`,
      folderId,
    );
    pageResults.push(result);

    if (!result.ok) {
      attempts.push(`${label} skip ${skip}: ${result.message}`);
      return { ...result, attempts, label, pageResults };
    }

    items.push(...result.items);
    attempts.push(`${label} skip ${skip}: ${result.items.length} item(s)`);

    if (result.items.length < odataPageSize) {
      return {
        ...result,
        items,
        payload: { value: items },
        ...
        diagnostic: {
          ...result.diagnostic,
          resultSummary: `${items.length} ${entitySet} item(s) returned across ${pageResults.length} page(s).`,
  ...
  return {
    ok: false as const,
    message: `${label} pagination stopped after ${maxCurrentUserFolderPages} page(s). ${attempts.join(' | ')}`,
    attempts,
    label,
    pageResults,
  };
...
async function runTenantFolderProbe(config: ProbeConnectionConfig, token: string | undefined) {
  const attempts: string[] = [];
  const pageResults: LiveRawProbeResult[] = [];
  const items: RawRecord[] = [];
  ...
  for (let pageIndex = 0; pageIndex < maxCurrentUserFolderPages; pageIndex += 1) {
    ...
    const result = await runODataProbeWithFallback(
      ...
      token,
      'Folders',
      `${tenantFolderQuery}&$skip=${skip}`,
      `${tenantFolderFallbackQuery}&$skip=${skip}`,
    );
    pageResults.push(result);

    if (!result.ok) {
      attempts.push(`${tenantFoldersLabel} skip ${skip}: ${result.message}`);
      return { ...result, attempts, label: tenantFoldersLabel, pageResults };
    }

    items.push(...result.items);
    attempts.push(`${tenantFoldersLabel} skip ${skip}: ${result.items.length} item(s)`);
    ...
    if (result.items.length < odataPageSize) {
      return {
        ...
        payload: { value: items },
        attempts,
        label: tenantFoldersLabel,
        pageResults,
        diagnostic: {
          ...result.diagnostic,
          resultSummary: `${items.length} tenant folder(s) returned across ${pageResults.length} page(s).`,
          responsePreview: items.length ? previewText(items[0]) : previewText(result.payload),
        },
      };
    }
  ...
  return {
    ok: false as const,
    message: `Tenant folder pagination stopped after ${maxCurrentUserFolderPages} page(s). ${attempts.join(' | ')}`,
    attempts,
    label: tenantFoldersLabel,
    pageResults,
  };
...
async function runTenantAdminProbe(config: ProbeConnectionConfig, token: string | undefined) {
  const attempts: string[] = [];
  const results: LiveRawProbeResult[] = [];
  let firstReadableInfo: ReturnType<typeof tenantAdminInfoFromPayload> | null = null;
  let firstReadableLabel = 'none';

  for (const candidate of currentUserRoleCandidates) {
    const result = await runJsonProbe(config, token, candidate.path, candidate.query);
    results.push(result);
    ...
    const info = tenantAdminInfoFromPayload(result.payload);
    attempts.push(`${candidate.label}: ${info.roleNames.length ? info.roleNames.join(', ') : 'no role names exposed'}`);
    if (!firstReadableInfo) {
      firstReadableInfo = info;
      firstReadableLabel = candidate.label;
    }
    if (info.tenantAdmin || info.roleNames.length) return { ...info, label: candidate.label, attempts, results };
  }

  if (firstReadableInfo) {
    return { ...firstReadableInfo, label: firstReadableLabel, attempts, results };
  }
...
export async function retrieveOrchestratorSnapshot(
  ...
  const tenantAdminProbe = await runTenantAdminProbe(config, token);
  apiCalls.push(...tenantAdminProbe.results.flatMap((result) => collectDiagnostics(result)));
  const tenantAdmin = tenantAdminProbe.tenantAdmin;
  const tenantAdminRoleNames = tenantAdminProbe.roleNames;
  messages.push(`Role API read attempts: ${tenantAdminProbe.attempts.join(' | ') || 'no role endpoint succeeded'}.`);
  if (tenantAdmin) {
    messages.push(`Orchestrator Administrator role enabled${tenantAdminRoleNames.length ? ` (${tenantAdminRoleNames.join(', ')})` : ''}; folder and machine discovery uses tenant-wide reads where available.`);
  }

  const currentUserFolderProbe = await runFolderProbe(config, token);
  apiCalls.push(...(currentUserFolderProbe.pageResults?.length ? currentUserFolderProbe.pageResults.flatMap((result) => collectDiagnostics(result)) : collectDiagnostics(currentUserFolderProbe)));
  const tenantFolderProbe = tenantAdmin ? await runTenantFolderProbe(config, token) : null;
  if (tenantFolderProbe) {
    apiCalls.push(...(tenantFolderProbe.pageResults?.length ? tenantFolderProbe.pageResults.flatMap((result) => collectDiagnostics(result)) : collectDiagnostics(tenantFolderProbe)));
  }
  const folderProbe = tenantFolderProbe?.ok ? tenantFolderProbe : currentUserFolderProbe;
  ...
  if (tenantAdmin && tenantFolderProbe && !tenantFolderProbe.ok) {
    messages.push(`Tenant-wide folder read failed, so discovery fell back to current-user folders: ${tenantFolderProbe.message}`);
  }
  if (folderProbe.ok) {
  ...
  const machineProbe = tenantAdmin
    ? await runPagedODataProbeWithFallback(config, token, 'Machines', machineQuery, machineFallbackQuery, undefined, 'tenant-wide Machines')
    : null;
  apiCalls.push(...(machineProbe?.pageResults.length ? machineProbe.pageResults.flatMap((result) => collectDiagnostics(result)) : collectDiagnostics(machineProbe)));
  if (tenantAdmin && machineProbe && !machineProbe.ok) messages.push(`Machine API read failed: ${machineProbe.message}`);

  const folderMachineProbes = folderDetails.length
    ? await runFolderedPagedODataProbe(config, token, folderDetails, 'Machines', machineQuery, machineFallbackQuery)
    : [];
  apiCalls.push(...folderMachineProbes.flatMap((probe) =>
    probe.result.pageResults.length
      ? probe.result.pageResults.flatMap((result) => collectDiagnostics(result))
      : collectDiagnostics(probe.result),
  ));
  ...
  const machines = mergeMachineSummaries([
    ...(tenantAdmin && machineProbe?.ok ? machineProbe.items.map((item) => summarizeMachine(item, sessions)) : []),
    ...folderScopedMachines,
  ]);
  if (!tenantAdmin) {
    messages.push('Machine selector limited to machines returned from the discovered folder list.');
  } else if (machineProbe?.ok) {
    messages.push(`Tenant-admin machine read mapped ${machineProbe.items.length} tenant-wide machine record(s) across ${machineProbe.pageResults.length} page(s).`);
  }
