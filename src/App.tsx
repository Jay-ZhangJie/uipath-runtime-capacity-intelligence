...
type DayInterval = 'hour' | 'quarter-hour';

const grains: TimeGrain[] = ['day', 'week', 'month'];
const dayIntervals: DayInterval[] = ['hour', 'quarter-hour'];
const quarterHourMinutes = ['00', '15', '30', '45'];
...
function formatSlotTime(hour: string, minute = '00') {
  return `${hour}:${minute}`;
}
...
function roundedLastThirtyDayWeekDates(date: string) {
  const lastThirtyDays = historicalDatesForRange('last-month', date);
  const startDate = weekDatesFor(lastThirtyDays[0] ?? date)[0];
  const endWeek = weekDatesFor(lastThirtyDays.at(-1) ?? date);
...
function dateOptionsForRange(range: DateRange, liveCalendar = false) {
  ...
  if (range === 'last-month') return historicalDatesForRange(range, initialTenantSlot.date);
  return quarterDates;
}
...
function singleCronNumber(field: string, min: number, max: number) {
  if (!/^\d+$/.test(field)) return null;
  const value = Number(field);
  return value >= min && value <= max ? value : null;
}

function describeCronList(field: string, values: Record<string, string>) {
  return field
    .split(',')
    .map((part) => values[part.toUpperCase()] ?? part)
    .join(', ');
}

function describeCronCadence(dayOfMonth: string, month: string, dayOfWeek: string) {
  const normalizedDow = dayOfWeek.toUpperCase();
  const monthNote = month === '*' ? '' : ` during month ${month}`;

  if (dayOfMonth !== '*' && dayOfWeek === '*') {
    return `Monthly on day ${dayOfMonth}${monthNote}`;
  }

  if (normalizedDow === '*' && dayOfMonth === '*') return `Every day${monthNote}`;
  if (['1-5', 'MON-FRI'].includes(normalizedDow)) return `Weekdays${monthNote}`;
  if (['0,6', '6,0', 'SUN,SAT', 'SAT,SUN'].includes(normalizedDow)) return `Weekends${monthNote}`;
  ...
  return `On ${describeCronList(dayOfWeek, dayNames)}${monthNote}`;
}

function parseCronSchedule(text: string): ScheduleParsePreview | null {
  const fields = text.trim().split(/\s+/);
  if (fields.length !== 5) return null;

  const [minuteField, hourField, dayOfMonthField, monthField, dayOfWeekField] = fields;
  const cronFieldPattern = /^[\dA-Z*,/\-]+$/i;
  if (!fields.every((field) => cronFieldPattern.test(field))) return null;

  const minute = singleCronNumber(minuteField, 0, 59);
  const hour = singleCronNumber(hourField, 0, 23);
  const time = hour !== null && minute !== null
    ? formatClock(hour, minute)
    : `Multiple times (${hourField}:${minuteField})`;
  const warnings = [
    minute === null || hour === null ? 'Multiple matching start times detected; preview summarizes the cadence only.' : '',
  ].filter(Boolean);

  return {
    source: 'CRON',
    cadence: describeCronCadence(dayOfMonthField, monthField, dayOfWeekField),
    time,
    startHour: hour !== null && minute !== null ? String(hour).padStart(2, '0') : undefined,
    confidence: warnings.length ? 'Medium' : 'High',
    notes: [
      `Parsed from five-field CRON: minute hour day-of-month month day-of-week.`,
      'Run simulation uses this parsed start time; recurring expansion across multiple dates is on the roadmap.',
    ],
    warnings,
  };
}

function parseNaturalLanguageSchedule(text: string, fallbackHour: string): ScheduleParsePreview {
  ...
  if (/\b(weekdays?|business days?|mon(day)?\s*(to|through|-)\s*fri(day)?|mon-fri)\b/.test(normalized)) {
    cadence = 'Weekdays';
  } else if (/\b(daily|every day|each day)\b/.test(normalized)) {
    cadence = 'Every day';
  } else if (/\b(month[-\s]?end|end of month|eom)\b/.test(normalized)) {
    cadence = 'Month end';
    notes.push('Mentions month-end demand; recurring expansion is on the roadmap.');
  } else if (/\b(monthly|every month)\b/.test(normalized)) {
    cadence = 'Monthly';
  } else if (/\b(weekly|every week)\b/.test(normalized)) {
    cadence = 'Weekly';
  }

  const dayMatches = Array.from(normalized.matchAll(/\b(mon|monday|tue|tues|tuesday|wed|wednesday|thu|thur|thurs|thursday|fri|friday|sat|saturday|sun|sunday)\b/g));
  if (!cadence && dayMatches.length) {
    const dayLabels = new Set(dayMatches.map((match) => match[0].slice(0, 3)));
    cadence = `On ${Array.from(dayLabels).join(', ')}`;
  }

  const explicitTime = normalized.match(/\b(?:at|around|by|from)?\s*(\d{1,2})(?::(\d{2}))?\s*(am|pm)\b/);
  if (explicitTime) {
  ...
  } else if (/\b(evening|night)\b/.test(normalized)) {
    time = 'Evening window';
  }

  if (/\b(queue spike|spike|surge|peak|high volume)\b/.test(normalized)) {
    notes.push('Mentions elevated volume; validate runtime demand before relying on the result.');
  }

  if (!cadence) {
    cadence = 'One-time or unclear recurrence';
    warnings.push('Could not infer a recurring cadence.');
  }

  if (!time) {
    time = `${fallbackHour}:00 current tenant hour`;
    warnings.push('No explicit start time found; using the current tenant hour.');
  }
  ...
  return {
    ...
    time,
    startHour,
    confidence: warnings.length ? (notes.length || cadence !== 'One-time or unclear recurrence' ? 'Medium' : 'Low') : 'High',
    notes: [
      ...notes,
      'Run simulation uses the parsed start time when available, plus runtime demand and duration below.',
    ],
    warnings,
  };
}

function parseScheduleDescription(scenario: WhatIfScenario): ScheduleParsePreview | null {
  const text = scenario.businessDetails.trim();
  if (!text) return null;
  return parseCronSchedule(text) ?? parseNaturalLanguageSchedule(text, scenario.preferredHour);
}

function applySingleScenarioImpact(buckets: RuntimeBucket[], scenario: WhatIfScenario, machineTemplateId: string) {
  ...
  return buckets.map((bucket) => {
    ...
    if (scenario.machineTemplateId !== 'all' && scenario.machineTemplateId !== bucket.machineTemplateId) return bucket;
...
function applyScenarioImpact(buckets: RuntimeBucket[], scenarios: WhatIfScenario[], machineTemplateId: string) {
  return scenarios.reduce(
    (impactedBuckets, scenario) => applySingleScenarioImpact(impactedBuckets, scenario, machineTemplateId),
    buckets,
  );
}
...
function buildScenarioAdvisorRecommendation(
  ...
  const detailHint = scenario.businessDetails.trim()
    ...
    : 'No schedule notes were provided, so this uses runtime demand, duration, and current capacity only.';
  const action =
  ...
  return {
    ...
    confidence,
    rationale: `${scenario.runtimeDemand} runtime(s) for ${scenario.durationMinutes} minutes. ${detailHint}`,
    followUps: [
...
export function App() {
  ...
  const [dayInterval, setDayInterval] = useState<DayInterval>('hour');
  ...
  const [submittedScenarios, setSubmittedScenarios] = useState<WhatIfScenario[]>([]);
  ...
  const machineTemplateCount = visibleMachineTemplates.length;
  const machineTemplateDetail = liveMode
    ? 'Template/inventory records from paged Machines API reads'
    : 'Sign in to load';
  ...
  const focusDateOptions = useMemo(
    () => Array.from(new Set([...dateOptions, ...quarterDates, ...submittedScenarios.map((scenario) => scenario.preferredDate)])).filter((date) =>
      dateOptions.includes(date) || date === selectedDate || date === scenarioForm.preferredDate || submittedScenarios.some((scenario) => scenario.preferredDate === date),
    ),
    [dateOptions, scenarioForm.preferredDate, selectedDate, submittedScenarios],
  );
  ...
  const heatmapMonthDates = useMemo(() => roundedLastThirtyDayWeekDates(heatmapAnchorDate), []);
  const projectedRangeBuckets = useMemo(
    () => applyScenarioImpact(rangeBaseBuckets, submittedScenarios, machineTemplateId),
    [machineTemplateId, rangeBaseBuckets, submittedScenarios],
  );
  const heatmapBuckets = useMemo(
    () => applyScenarioImpact(templateBaseBuckets, submittedScenarios, machineTemplateId),
    [machineTemplateId, submittedScenarios, templateBaseBuckets],
  );
  const submittedScenarioImpacts = useMemo(
    () => submittedScenarios
      .map((scenario) => ({ scenario, impact: scenarioImpactSummary(scenario, templateBaseBuckets, heatmapBuckets) }))
      .filter((entry): entry is { scenario: WhatIfScenario; impact: NonNullable<ReturnType<typeof scenarioImpactSummary>> } => Boolean(entry.impact)),
    [heatmapBuckets, submittedScenarios, templateBaseBuckets],
  );
  const recommendations = useMemo(() => {
    const base = buildRecommendations(visibleRisks);
    if (!submittedScenarios.length) return base;

    const scenarioRecs: Recommendation[] = submittedScenarios.map((scenario, index) => ({
      id: `what-if-impact-${index}`,
      type: 'move-schedule',
      owner: 'Release Manager',
      title: `${scenario.solutionName || `Simulation ${index + 1}`} changes ${formatDate(scenario.preferredDate)} ${scenario.preferredHour}:00 demand`,
      impact: `Adds ${scenario.runtimeDemand} runtime(s) for about ${scenario.durationMinutes} minutes. Review the highlighted heatmap tiles before choosing this slot.`,
      confidence: 'Medium',
      basis: liveMode
    ...
    return [...scenarioRecs.reverse(), ...base];
  }, [liveMode, submittedScenarios, visibleRisks]);
  ...
  const tenantAdminRoleDetail = liveProbe?.tenantAdminRoleNames.length
    ? liveProbe.tenantAdminRoleNames.join(', ')
    : 'Orchestrator Administrator role detected from Orchestrator role API';
  const dataSignals: DataSignal[] = (() => {
    ...
    if (liveProbe?.status === 'connected' || liveProbe?.status === 'partial') {
      return [
        ...(liveProbe.tenantAdmin ? [{
          id: 'tenant-admin-mode',
          tone: 'ok' as const,
          title: 'Orchestrator Administrator enabled',
          detail: `Admin mode is using tenant-wide folder and machine discovery. ${tenantAdminRoleDetail}.`,
  ...
  function submitScenario(event: FormEvent<HTMLFormElement>) {
    ...
    const schedulePreview = parseScheduleDescription(scenarioForm);
    const scenarioToSubmit = {
      ...scenarioForm,
      preferredHour: schedulePreview?.startHour ?? scenarioForm.preferredHour,
    };
    setSubmittedScenarios((current) => [...current, scenarioToSubmit]);
    alignSelectedSlot({ date: scenarioToSubmit.preferredDate, hour: scenarioToSubmit.preferredHour }, 'day');
  }
  ...
  function downloadAggregatedData() {
    ...
    downloadJsonFile(filename, {
      ...
      filters: {
        ...
        dateRange,
        timeGrain: grain,
        dayInterval: grain === 'day' ? dayInterval : null,
        riskView,
      ...
      <header className="topbar">
        <div className="brand">
          ...
          <div className="brand-copy">
            ...
            <p>Read-only UiPath license, runtime and schedule heatmap tool</p>
          </div>
        </div>
        ...
        <div className="topbar-actions">
          ...
          {liveProbe?.tenantAdmin ? (
            <div className="admin-mode-chip" title={`Admin mode is using tenant-wide discovery. ${tenantAdminRoleDetail}`}>
              <ShieldCheck size={16} />
              Orchestrator Administrator enabled
            </div>
          ) : null}
          <button
      ...
      <main>
        <section className="filters-panel" aria-label="Permission scoped filters">
          ...
          <label>
            ...
            <select value={defaultDateRange} disabled aria-label="Date range locked to Last 30 days">
              {dateRanges.map((item) => (
          ...
          <div className="filter-action">
            <div className="filter-action-buttons">
              ...
              <button
                className="button secondary download-action"
                type="button"
          ...
          <div className="main-column">
                ...
                <div>
                    ...
                    {grain === 'day' ? (
                      <div className="day-toolbar-controls">
                        <SegmentedControl
                          ...
                          labelFormatter={(item) => (item === 'hour' ? 'Hours' : '15 mins')}
                          ariaLabel="Day view interval"
                        />
                        <label className="inline-filter">
                          ...
                          <select
                            value={selectedDate}
                            onChange={(event) => {
                              selectTile({ ...selectedTile, date: event.target.value });
                            }}
                          >
                  ...
                  {selectedBucket ? (
                    <div className="tile-detail">
                      <strong>{formatDate(selectedTile.date, 'long')} {formatSlotTime(selectedTile.hour, selectedTile.minute)} detail</strong>
                  ...
                  {submittedScenarioImpacts.map(({ scenario, impact }, index) => (
                    <ScenarioImpactCard
                      impact={impact}
                      key={`${scenario.solutionName}-${scenario.preferredDate}-${scenario.preferredHour}-${index}`}
                      scenario={scenario}
                    />
                  ))}
                </div>
            ...
            <section className="panel">
              ...
              <div className="inventory-summary">
                <MiniStat label="Machine Templates" value={machineTemplateCount} detail={machineTemplateDetail} />
...
function DayTimeline({
  ...
  const slotMinutes = dayInterval === 'quarter-hour' ? quarterHourMinutes : ['00'];
  const intervalSummary = dayInterval === 'quarter-hour'
    ? `96 15-minute slots derived from hourly ${liveHistorical ? 'peak consumed' : 'projected'} buckets in ${tenantTimezoneLabel}`
      ...
      <div className="day-period-grid">
        {dayPeriods.map((period) => (
          <section className="day-period" key={period.label}>
            <div className="period-label">{dayInterval === 'quarter-hour' ? `${period.label} - 15 min slots` : period.label}</div>
            <div className={`period-hours ${dayInterval === 'quarter-hour' ? 'quarter-hour-grid' : ''}`}>
              {period.hours.flatMap((hour) => slotMinutes.map((minute) => {
                const bucket = bucketFor(date, hour, buckets);
                const slotLabel = formatSlotTime(hour, minute);
                if (!bucket) {
                  return (
                    <div className={`heatmap-empty ${future ? 'future-date' : ''}`} key={`${date}-${hour}-${minute}`}>
                      <span>{slotLabel}</span>
                      <small>No data</small>
                    </div>
                  );
                ...
                const isSelected = selectedTile.date === bucket.date &&
                  selectedTile.hour === bucket.hour &&
                  (dayInterval === 'hour' || (selectedTile.minute ?? '00') === minute);
                const demand = future ? bucket.projectedDemand : liveHistorical ? bucket.observedDemand : bucket.projectedDemand;
                return (
                  <button
                    className={`hour-card ${dayInterval === 'quarter-hour' ? 'quarter-card' : ''} ${riskTone(risk)} ${isSelected ? 'selected' : ''}`}
                    key={`${bucket.date}-${bucket.hour}-${minute}`}
                    onClick={() => onTileClick({ date: bucket.date, hour: bucket.hour, minute })}
                    title={`${formatDate(bucket.date)} ${slotLabel} - ${percent}% - ${bucket.topDrivers.join(', ')}`}
                    type="button"
                  >
                    <span>{slotLabel}</span>
...
function WhatIfPlanner({
  ...
  const schedulePreview = parseScheduleDescription(scenario);

  return (
    <form className="what-if-panel" onSubmit={onSubmit}>
      <div>
        <h3>Schedule Simulation</h3>
        <p>Simulate a new use case against projected runtime demand. All schedule inputs use {tenantTimezoneLabel}.</p>
      </div>
      <label>
      ...
      <label>
        <span>Describe automation schedule</span>
        <textarea
          placeholder="Use natural language or CRON, for example: every weekday at 8:00 AM, 0 8 * * 1-5, month-end queue spike"
          rows={3}
          value={scenario.businessDetails}
          onChange={(event) => onChange({ ...scenario, businessDetails: event.target.value })}
        />
      </label>
      {schedulePreview ? <SchedulePreviewCard preview={schedulePreview} /> : null}
      <label>
        <span>Folder</span>
      ...
      <div className="form-actions">
        <button className="button secondary" type="button" disabled title="AI schedule recommendations are on the roadmap.">
          <Sparkles size={16} />
          AI on roadmap
        </button>
        <button className="button primary" type="submit">Run simulation</button>
...
function SchedulePreviewCard({ preview }: { preview: ScheduleParsePreview }) {
  return (
    <section className="schedule-preview" aria-label="Parsed schedule preview">
      <div className="schedule-preview-header">
        <span>Parsed preview</span>
        <strong>{preview.source} - {preview.confidence}</strong>
      </div>
      <div className="schedule-preview-grid">
        <div>
          <span>Cadence</span>
          <strong>{preview.cadence}</strong>
        </div>
        <div>
          <span>Start</span>
      ...
      {preview.warnings.map((warning) => (
        <small className="schedule-warning" key={warning}>{warning}</small>
      ))}
    </section>
  );
}
