...
React + Vite + TypeScript Coded Web App with browser OAuth, saved single-tenant connection profiles, and a modular live Orchestrator read connector. The current live path validates non-confidential External App sign-in, required read scopes, folder discovery, tenant license info, sessions, and scoped reads for jobs, machines, and triggers where the target tenant allows them. Machine inventory reads are paged and constrained to the discovered folder list for regular users.

When the signed-in user has the UiPath default **Orchestrator Administrator** role, the app enables admin mode, shows an **Orchestrator Administrator enabled** status, and uses tenant-wide folder and machine discovery where the read-only APIs allow it. No other custom or generic administrator role name enables admin mode.
...
After `npm run dev`, open `http://localhost:5173/` and click **Sign in**. The Diagnose button and panel should show whether OAuth connected, which connector scope was used, whether the exact Orchestrator Administrator role was detected, and whether the read-only license, folder, recent active session, job, machine, and trigger API reads succeeded or returned partial-access warnings.
...
- A signed-in pilot user whose existing Orchestrator permissions represent the folders and machine templates the customer wants analyzed, or an approved Orchestrator Administrator pilot user when tenant-wide validation is explicitly intended.
...
- Confirm folder and paged machine-template discovery works with customer permissions, including the regular-user folder-scoped view and the Orchestrator Administrator tenant-wide view when applicable.
...
The app is intentionally read-only. These scopes allow the browser app to read execution/job history, folders, tenant license/settings data, machine templates, robot/session signals, and user metadata needed for runtime-lane mapping and exact Orchestrator Administrator role detection. Add `Insights` and `Insights.RealTimeData` later if replacing job-derived demand with Insights Real-Time Monitoring metrics.

## Documentation

- [PDD](docs/pdd.md)
- [SDD](docs/sdd.md)
- [Customer Feature Feedback](docs/customer-feature-feedback.md)
