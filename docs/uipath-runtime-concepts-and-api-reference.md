...
The app uses a non-confidential External Application and browser OAuth. It requests user-delegated, read-only scopes and respects the signed-in user's Orchestrator permissions.
...
| --- | --- | --- | --- | --- | --- |
| Folder discovery | `api/Folders/GetAllForCurrentUser` | Tenant/user | `take=200&skip=<offset>` paged at 200 records | No | Primary and only folder discovery path. Returns folders the signed-in user can access and avoids tenant-wide `odata/Folders` reads. |
...
| Machine inventory | `odata/Machines` | Tenant | `$select=<runtime capacity fields>&$top=100&$orderby=Name asc` | No | Reads machine/template identity, folder, and runtime slot fields, with a broad-query fallback if a tenant rejects the projection. |
