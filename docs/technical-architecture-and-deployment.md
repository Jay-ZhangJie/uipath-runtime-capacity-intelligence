...
- Permission-scoped tenant and folder lists.
- Current machine template and runtime configuration.
...
| `MachineTemplateInventorySnapshot` | Template runtime slots, connected machines, configured max capacity, folders, runtime type, and host/session monitoring context by template. |
...
- Load license info, folders, jobs, triggers, machines, sessions, and machine-template signals by API where tenant permissions allow.
...
| Pilot user assignment | Restrict visibility to customer-approved folders and machine templates. |
