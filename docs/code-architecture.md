...
| Orchestrator retrieval | `src/lib/orchestratorRetriever.ts` | License info, folder, session, job, machine, and trigger reads from UiPath Orchestrator. |
...
4. `orchestratorRetriever.ts` reads tenant license info, folders, sessions, and machine inventory first, then performs folder-scoped reads for jobs and triggers where possible.
...
3. Confirm the signed-in user has folder assignments in the selected tenant.
4. Confirm the External App includes read scopes for folders, execution/jobs, machines, robots/sessions, licenses/settings, and triggers if trigger reads are enabled.
